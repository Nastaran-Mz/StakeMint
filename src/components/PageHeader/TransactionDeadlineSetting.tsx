  ...
  return (
    <Box mb="16px">
      <Flex alignItems="center" mb="8px">
        <Text color='black' bold>{translateString(90, 'Transaction deadline')}</Text>
        <QuestionHelper
          text={translateString(188, 'Your transaction will revert if it is pending for more than this long.')}
        />
      </Flex>
      <Field>
        <Input style={{backgroundColor:'#D5D5D5',color:'#0FD17A'}} type="number" step="1" min="1" value={value} onChange={handleChange} />
        <Text color='balck' fontSize="14px" ml="8px">
          Minutes
        </Text>
      </Field>
      {error && (
