...
const RecentTransactionsModal = ({ onDismiss = defaultOnDismiss, translateString }: RecentTransactionsModalProps) => {
  ...
  return (
    <Modal 
     title={translateString(1202, 'Recent transactions')} onDismiss={onDismiss}>
      {!account && (
        <Flex justifyContent="center" flexDirection="column" alignItems="center">
          <Text color='black' mb="8px" bold>
            Please connect your wallet to view your recent transactions
          </Text>
          <Button style={{backgroundColor:'#0FD17A',color:'white'}} variant="tertiary" scale="sm" onClick={onDismiss}>
            Close
          </Button>
        </Flex>
      )}
