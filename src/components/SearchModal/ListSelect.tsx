...
const ListRow = memo(function ListRow({ listUrl, onBack }: { listUrl: string; onBack: () => void }) {
  ...
  return (
    <Row key={listUrl} align="center" padding="16px" id={listUrlRowHTMLId(listUrl)}>
      {list.logoURI ? (
        <ListLogo style={{ marginRight: '1rem',color:'black' }} logoURI={list.logoURI} alt={`${list.name} list logo`} />
      ) : (
        <div style={{ width: '24px', height: '24px', marginRight: '1rem' }} />
      )}
      <Column style={{ flex: '1' }}>
        <Row>
          <Text color='black' bold={isSelected} fontSize="16px" style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {list.name}
          </Text>
        </Row>
        <Row
      ...
      {isSelected ? (
        <Button disabled style={{ width: '5rem', minWidth: '5rem',color:'black' }}>
          Selected
        </Button>
      ) : (
        <>
    ...
    <Column style={{ width: '100%', flex: '1 1' }}>
      <PaddedColumn>
        <RowBetween>
          ...
          <Text color='black' fontSize="20px">{TranslateString(1208, 'Manage Lists')}</Text>
      ...
      <PaddedColumn gap="14px">
        ...
        <Row>
          ...
          />
          <Button onClick={handleAddList} style={{ maxWidth: '4em', marginLeft: '1em',color:'black' }} disabled={!validUrl}>
            Add
          </Button>
        </Row>
        {addError ? (
